...
import llm_client.api.default.paraphrasing_api_v1_paraphrasing_post
from llm_client.models import TextInDTO, TextInDTOParams, TextCorrectInDTO
import pdf_client.api.default as pdf_client_handles
import pdf_client.api.default.generation_api_v1_pdf_post
from pdf_client.models import PDFInDTO, PDFInDTOParams
...
pdf_client = AuthenticatedClient(
    os.environ.get("PDF_SERVICE_URL"),
    create_service_token(Service.REPORT)
)
...
async def update_report(
    ...
    db_report.status = ReportStatusType.UPDATING

    for status in db_report.statuses:
        if status.service_id == Service.LLM:
            status.status = ReportStatusType.PENDING  # TODO Тут кажется нелогичным сервису присваивать статус из ReportStatus
            session.add(status)
    ...
    if report_in_dto.params:
        ...
        for new_param in report_in_dto.params:
            ...
            new_params[new_param.name] = new_param.data.dict()["value"]
        ...
        old_report_text: ReportText = result.scalar_one_or_none()

        # result: List[ReportParameter] = await ReportParameter.select_latest(session, report_id=report_id)
        # old_params = {}
        # for old_param in result:
        #     old_params[old_param.name] = old_param.value
        ...
        async def send_correction():
            ...
            await llm_client_handles.correct_api_v1_correct_post.asyncio_detailed(
                ...
                body=TextCorrectInDTO(
                    report_id=report_id,
                    params=TextInDTOParams.from_dict(new_params),
                    text=old_report_text.text
                )
            )
...
async def update_report(
    ...
    max_version_params = (await ReportParameter.get_max_version(session, report_id)) or 0
    ...
    if report_in_dto.params:
        ...
        for param in final_params:
            param.version = max_version_params
            param.report_id = report_id
        session.add_all(final_params)
    ...
    if report_in_dto.text:
        ...
        max_version_text = await ReportText.get_max_version(session, report_id)

        if max_version_text is None:
            max_version_text = 0
        else:
            max_version_text += 1
...
async def send_text_generation(
    ...
    # TODO result = await ReportParameter.select_latest(session, report_id=db_report.id)
    # TODO Может выполнять запрос так как выше? ^
    # TODO И вообще тут какой-то бред, почему выбирается 0 версия а не полсдения, ведь после ручного изменения
    # параметров будет совсем не корректно работать
    ...
    for param in result:
        params_dict[param.name] = param.data['value']
...
@app.post("/api/v1/report/{report_id}/rephrase", response_model=ReportDetailedOutDto)
async def rephrase_text(
    ...
    async def send_paraphrasing():
        logger.info("Sending text to LLM for paraphrasing")

        await llm_client_handles.paraphrasing_api_v1_paraphrasing_post.asyncio_detailed(
            client=llm_client,
            body=TextCorrectInDTO(
                report_id=report_id,
                params=TextInDTOParams.from_dict({}),
                text=old_report_text.text
            )
        )

    asyncio.ensure_future(send_paraphrasing())
...
async def update_active_text(
    ...
    new_active_text = await ReportText.select_version(session, report_id, version)
    new_active_text.active = True
    session.add(new_active_text)
...
@app.post("/api/v1/report/{report_id}/pdf", response_class=StreamingResponse)
async def get_pdf(
        report_id: int,
        current_user: CurrentAuthData,
        session: AsyncSession = Depends(get_session),
    ...
    max_version = await ReportParameter.get_max_version(session, report_id=db_report.id)
    ...
    async def fetch_image(image_type: ReportImageType):
        try:
            result = await session.execute(
                select(ReportImage)
                .where(ReportImage.report_id == report_id)
            ...
            # mask_b64 = base64.b64encode(image.data).decode('ascii')
            img_encoded = base64.b64encode(image.data).decode('ascii')
            return img_encoded
        except Exception as e:
            logger.error(f"Error fetching {image_type}: {str(e)}")
            return None

    # Параллельное получение изображений
    source_content, od_content, mac_content, av_content = await asyncio.gather(
        fetch_image(ReportImageType.SOURCE),
        fetch_image(ReportImageType.OD_CONTOUR),
        fetch_image(ReportImageType.MAC_CONTOUR),
        fetch_image(ReportImageType.AV_SEGMENTATION)
    )

    result_text = await session.execute(
        select(ReportText).where((ReportText.report_id == report_id) & (ReportText.active == True)))
    report_text: ReportText = result_text.scalar_one_or_none()
    text = report_text.text

    logger.info("Sending data to PDF for creation")

    response = await pdf_client_handles.generation_api_v1_pdf_post.asyncio_detailed(
        client=pdf_client,
        body=PDFInDTO(
            report_id=report_id,
            params=PDFInDTOParams.from_dict(params_dict),
            source_img=source_content,
            od_img=od_content,
            mac_img=mac_content,
            av_img=av_content,
            text=text
    ...
    return StreamingResponse(content=BytesIO(pdf_content),
