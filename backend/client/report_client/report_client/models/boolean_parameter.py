from collections.abc import Mapping
from typing import TYPE_CHECKING, Any, Literal, TypeVar, Union, cast

from attrs import define as _attrs_define
from attrs import field as _attrs_field

from ..types import UNSET, Unset

if TYPE_CHECKING:
    from ..models.boolean_parameter_data import BooleanParameterData


T = TypeVar("T", bound="BooleanParameter")


@_attrs_define
class BooleanParameter:
    """
    Attributes:
        name (str):
        data (BooleanParameterData):
        type_ (Union[Literal['boolean'], Unset]):  Default: 'boolean'.
    """

    name: str
    data: "BooleanParameterData"
    type_: Union[Literal["boolean"], Unset] = "boolean"
    additional_properties: dict[str, Any] = _attrs_field(init=False, factory=dict)

    def to_dict(self) -> dict[str, Any]:
        name = self.name

        data = self.data.to_dict()

        type_ = self.type_

        field_dict: dict[str, Any] = {}
        field_dict.update(self.additional_properties)
        field_dict.update(
            {
                "name": name,
                "data": data,
            }
        )
        if type_ is not UNSET:
            field_dict["type"] = type_

        return field_dict

    @classmethod
    def from_dict(cls: type[T], src_dict: Mapping[str, Any]) -> T:
        from ..models.boolean_parameter_data import BooleanParameterData

        d = dict(src_dict)
        name = d.pop("name")

        data = BooleanParameterData.from_dict(d.pop("data"))

        type_ = cast(Union[Literal["boolean"], Unset], d.pop("type", UNSET))
        if type_ != "boolean" and not isinstance(type_, Unset):
            raise ValueError(f"type must match const 'boolean', got '{type_}'")

        boolean_parameter = cls(
            name=name,
            data=data,
            type_=type_,
        )

        boolean_parameter.additional_properties = d
        return boolean_parameter

    @property
    def additional_keys(self) -> list[str]:
        return list(self.additional_properties.keys())

    def __getitem__(self, key: str) -> Any:
        return self.additional_properties[key]

    def __setitem__(self, key: str, value: Any) -> None:
        self.additional_properties[key] = value

    def __delitem__(self, key: str) -> None:
        del self.additional_properties[key]

    def __contains__(self, key: str) -> bool:
        return key in self.additional_properties
